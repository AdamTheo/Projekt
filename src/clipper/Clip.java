    ...
    public Polygon clip() {
        ...
        for (int i = 0; i < edgesClipper.size(); i++) {
            ...
            for (int j = 0; j < polygon.size(); j++) {
                ...
                // orientace, pokud je mensi jak 0 je uvnitr
                position1 = (x2 - x1) * (y3 - y1) - (y2 - y1) * (x3 - x1) < 0;
                position2 = (x2 - x1) * (y4 - y1) - (y2 - y1) * (x4 - x1) < 0;
                // Nyni rozhodovaci cast Sutherland-Hodgemana algoritmu

                if (position1 && position2) { // Prvni pripad, obe jsou uvnitr
                    result.add(new Point(x4, y4));
                }
                if (position1 && !position2) { // Druhy pripad, druhy je venku
                    result.add(intersection(x1, y1, x2, y2, x3, y3, x4, y4));
                }
                if (!position1 && position2) { // Treti pripad, prvni je venku
                    result.add(intersection(x1, y1, x2, y2, x3, y3, x4, y4));
                    result.add(new Point(x4, y4));
                }
            ...
            for (Point point : result) {
                polygon.addPoint(point); // orizli jsme vsechny hrany o jednu
